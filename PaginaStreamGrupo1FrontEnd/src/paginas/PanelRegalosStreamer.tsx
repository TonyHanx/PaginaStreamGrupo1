...
const PanelRegalosStreamer: React.FC = () => {
  ...
  const [tipoRegalo, setTipoRegalo] = useState<"imagen" | "audio">("imagen"); 
  ...
  const obtenerStreamerId = (): string => {
    ...
    if (usuarioStr) {
      ...
      console.log('🔑 [obtenerStreamerId] Usuario completo:', usuario);
      // Prioridad: streamerId > id > userId > username
      const streamerId = usuario.streamerId || usuario.id || usuario.userId || usuario.username || "default";
      console.log('🆔 [obtenerStreamerId] StreamerId final:', streamerId);
      return streamerId;
    }
    console.warn('⚠️ [obtenerStreamerId] No se encontró USUARIO en sessionStorage');
    return "default";
  };

  const cargarRegalos = async () => {
    const streamerId = obtenerStreamerId();
    console.log('🆔 [cargarRegalos] StreamerId obtenido:', streamerId);
    console.log('📄 [cargarRegalos] USUARIO en sessionStorage:', sessionStorage.getItem('USUARIO'));
    console.log('🔄 [cargarRegalos] Iniciando carga para streamer:', streamerId);
    const regalos = await obtenerRegalosDeStreamer(streamerId);
    console.log('📦 [cargarRegalos] Regalos obtenidos:', regalos.length, regalos);
    setRegalosPersonalizados(regalos);
    console.log('✅ [cargarRegalos] Estado actualizado con', regalos.length, 'regalos');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // según el tipo, exigimos imagen o audio
    const urlValida =
      tipoRegalo === 'imagen'
        ? imagenUrl.trim()
        : audioUrl.trim();
    ...
    if (!nombre || !urlValida || isNaN(precioNum) || isNaN(puntosNum)) {
    ...
    if (modoEdicion && regaloEditando) {
      ...
      const exito = editarRegalo(regaloEditando.id, nombre, precioNum, imagenUrl, puntosNum, audioUrl);
      if (exito) {
      ...
      const nuevoRegalo = await crearRegalo(nombre, precioNum, imagenUrl, puntosNum, streamerId, audioUrl);
  ...
  const handleEditar = (regalo: Regalo) => {
    ...
    setAudioUrl(regalo.audioUrl || "");
    setPuntos(regalo.puntos.toString());
    // Si tiene audioUrl, asumimos que es de audio; si no, imagen
    setTipoRegalo(regalo.audioUrl ? 'audio' : 'imagen'); 
    setModoEdicion(true);
  };
        ...
        <div className="panel-regalos__formulario">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              <div className="tipo-regalo-options">
                <label>
                  <input
                    type="radio"
                    value="imagen"
                    checked={tipoRegalo === 'imagen'}
                    onChange={() => setTipoRegalo('imagen')}
                  />
                  Imagen
                </label>
                <label>
                  <input
                    type="radio"
                    value="audio"
                    checked={tipoRegalo === 'audio'}
                    onChange={() => setTipoRegalo('audio')}
                  />
                  Audio (sonido corto)
                </label>
              </div>
            ...
            {tipoRegalo === 'audio' && (
              <div className="form-group">
                <label htmlFor="audioUrl">URL del Audio del Regalo:</label>
                <input
                  type="url"
                  id="audioUrl"
                  value={audioUrl}
                  ...
                  required={tipoRegalo === 'audio'}
                />
                <small style={{ color: '#a0aec0', fontSize: '0.85rem', marginTop: '0.25rem', display: 'block' }}>
                  Formatos: MP3, OGG, WAV · Duración recomendada: 2-5 segundos
                </small>
              ...
              {regalosPersonalizados.map((regalo) => (
                <div key={regalo.id} className="regalo-card regalo-card--personalizado">
                    ...
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src =
                          'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64"><text y="50" font-size="48">🎁</text></svg>';
                      }}
                    />
                  )}

                  {!regalo.imagenUrl && regalo.audioUrl && (
                    <div className="regalo-audio-preview">
                      🔊 Audio
                    </div>
                  )}
                </div>
                  ...
                  <div className="regalo-info">
                    ...
                    <p>Puntos: {regalo.puntos} ⭐</p>
                    <p>
                    Tipo: {regalo.audioUrl ? "Audio" : "Imagen"}
                    </p>
                  </div>
